...
const StudentAttendance = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const attendanceRes = await api.get(`/api/attendance?classId=${filters.class}&date=${filters.date}`);
            ...
            {students.length > 0 && (
                <Card className="p-0 border-none shadow-3xl shadow-slate-100 bg-white rounded-3xl overflow-hidden animate-in slide-in-from-bottom duration-500">
                    <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                        <div>
                            <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">Attendance List</h3>
                            <p className="text-[10px] font-bold text-slate-400 mt-1">Mark attendance for students below</p>
                        </div>
                        <div className="flex space-x-3">
                            <Button onClick={() => markAll('Present')} className="bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100 px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all">Mark All Present</Button>
                            <Button onClick={() => markAll('Absent')} className="bg-rose-50 text-rose-600 border border-rose-100 hover:bg-rose-100 px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all">Mark All Absent</Button>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/50">
                                <tr>
                                    ...
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest italic tracking-tight">Roll No</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {students.map((student, idx) => (
                                    <tr key={student._id} className="hover:bg-slate-50/30 transition-colors group">
                                        ...
                                        <td className="px-8 py-4 text-xs font-bold text-slate-500">{student.roll || '-'}</td>
                                        ...
                                        <td className="px-8 py-4">
                                            <div className="flex justify-center space-x-2">
                                                {[
                                                    ...
                                                    { id: 'Half Day', label: 'F', icon: Clock, bg: 'bg-indigo-50', text: 'text-indigo-500', active: 'bg-indigo-500 text-white' }
                                                ].map(st => (
                                                    <button
                                                        key={st.id}
