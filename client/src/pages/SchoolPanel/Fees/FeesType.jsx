...
const FeesType = () => {
    ...
    const fetchTypes = async () => {
        try {
            const { data } = await api.get('/api/fees/types');
            setTypes(data);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showToast('Fee Type added successfully!');
            setFormData({ name: '', feeGroup: '', description: '', stream: '' });
            fetchTypes();
        } catch (error) {
            console.error('Error adding fee type:', error);
            showToast('Error adding fee type', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/api/fees/types/${id}`);
            showToast('Fee Type deleted successfully!');
            fetchTypes();
        } catch (error) {
            console.error('Error deleting fee type:', error);
            showToast('Error deleting fee type', 'error');
        }
    };
    ...
    const filteredTypes = types.filter(type =>
        ...
        (type.feeGroup && type.feeGroup.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-12 text-slate-800">
            <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-xl font-black text-slate-800 tracking-tight italic uppercase">Fees Type</h1>
                    <div className="flex space-x-2 text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">
                        ...
                        <span>Fees</span>
                        <span>|</span>
                        <span className="text-[#1C1C1C]">Fees Type</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                ...
                <Card className="p-8 border-none shadow-3xl shadow-slate-100 bg-white rounded-[32px] overflow-hidden sticky top-24">
                    <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest mb-8">Add Fees Type</h3>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1 italic">
                                FEES GROUP <span className="text-rose-500">*</span>
                            </label>
                ...
                <div className="lg:col-span-3">
                    <Card className="p-10 border-none shadow-3xl shadow-slate-100 bg-white rounded-[32px] overflow-hidden relative">
                        <div className="flex flex-col md:flex-row justify-between items-center mb-10 space-y-4 md:space-y-0">
                            <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">Fees Type List</h3>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-slate-50 italic text-slate-400">
                                        ...
                                        <th className="text-left py-5 px-6 text-[10px] font-black uppercase tracking-widest">
                                            <div className="flex items-center space-x-1"><span>↓Fees Group</span></div>
                                        </th>
                                        <th className="text-left py-5 px-6 text-[10px] font-black uppercase tracking-widest">
                                ...
                                <tbody>
                                    {filteredTypes.map((type) => (
                                        <tr key={type._id} className="group hover:bg-slate-50/50 transition-all border-b border-slate-50 last:border-0">
                                            <td className="py-5 px-6 text-[11px] font-bold text-slate-600 italic uppercase">{type.name}</td>
                                            <td className="py-5 px-6 text-[11px] font-bold text-slate-400 italic uppercase">
                                                {type.feeGroup?.name || '-'}
                                            </td>
                                            <td className="py-5 px-6 text-[11px] font-bold text-slate-400 italic">{type.description}</td>
                                            <td className="py-5 px-6 text-right">
                                                <div className="flex items-center justify-end space-x-2">
                                                    ...
                                                    <button onClick={() => handleDelete(type._id)} className="p-2 text-slate-300 hover:text-red-500 rounded-full transition-all">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
                                    ...
                                    {filteredTypes.length === 0 && (
                                        <tr>
                                            <td colSpan="4" className="py-8 text-center text-xs text-slate-400 italic font-bold">No fees types found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <div className="mt-8 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <span>Showing {filteredTypes.length} entries</span>
                        </div>
