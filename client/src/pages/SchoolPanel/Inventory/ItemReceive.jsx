...
const ItemReceive = () => {
    ...
    const [rows, setRows] = useState([
        { itemId: '', unitPrice: '', quantity: '', subTotal: 0 }
    ]);
        ...
        const fetchData = async () => {
            try {
                const [itemsRes, suppliersRes, storesRes] = await Promise.all([
                    ...
                    api.get('/api/inventory/supplier'),
    ...
    const addRow = () => {
        setRows([...rows, { itemId: '', unitPrice: '', quantity: '', subTotal: 0 }]);
    };
    ...
    const handleReceive = async () => {
        if (!headerData.supplier || !headerData.store || rows.some(r => !r.itemId || !r.quantity || !r.unitPrice)) {
            return showToast('Please fill all required fields', 'warning');
        }

        setLoading(true);
        try {
            // In a real application, you might want a specialized endpoint for bulk receive
            // For now, we'll iterate or use the existing transaction endpoint if it supports arrays
            // Assuming the backend needs multiple calls or a single bulk call
            for (const row of rows) {
                await api.post('/api/inventory/transaction', {
                    ...
                    transactionHead: headerData.expenseHead,
            ...
            // Reset form
            setRows([{ itemId: '', unitPrice: '', quantity: '', subTotal: 0 }]);
            setTotalPaid(0);
            setIsFullPaid(false);
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20 px-4">
            ...
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-xl font-black text-slate-800 tracking-tight uppercase">Item Receive</h1>
                <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    <span className="hover:text-primary cursor-pointer transition-colors">Dashboard</span>
                    <span>|</span>
                    <span className="hover:text-primary cursor-pointer transition-colors">Inventory</span>
                    <span>|</span>
                    <span className="text-slate-500">Item Receive</span>
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Left Section: Receive Details */}
                <Card className="lg:col-span-1 p-8 border-none shadow-snow-lg bg-white rounded-[32px] h-fit">
                    ...
                    <div className="space-y-6">
                        <Select
                            label="Expense Head"
                            name="expenseHead"
                            value={headerData.expenseHead}
                            onChange={handleHeaderChange}
                            options={['Inventory Expense', 'Office Supply', 'Furniture Purchase']}
                ...
                {/* Right Section: Item Receive Table */}
                <Card className="lg:col-span-2 p-8 border-none shadow-snow-lg bg-white rounded-[32px] flex flex-col min-h-[600px]">
                        ...
                        <table className="w-full border-separate border-spacing-y-4">
                            <thead>
                                <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                    ...
                                    <th className="text-left px-4">Unit Price *</th>
                    ...
                    <div className="flex justify-center mt-12 mb-4">
                        ...
                        >
                            <span>{loading ? 'PROCESSING...' : '✓ RECEIVE'}</span>
                        </Button>
                    </div>
                </Card>
            </div>
