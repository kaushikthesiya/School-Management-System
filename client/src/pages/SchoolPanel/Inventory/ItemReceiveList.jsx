...
const ItemReceiveList = () => {
    ...
    const fetchTransactions = async () => {
        ...
        try {
            ...
            // Filter only "Receive" type for this page
            setTransactions(res.data.filter(t => t.transactionType === 'Receive'));
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20 px-4">
            ...
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-xl font-black text-slate-800 tracking-tight uppercase">Item Receive List</h1>
                <div className="flex flex-col items-end space-y-4">
                    <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        ...
                        <span>|</span>
                        <span className="text-slate-500">Item Receive List</span>
                    </div>
                    <Button
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-50">
                                ...
                                <th className="text-left py-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">↓ Supplier Company Name</th>
                        ...
                        <tbody className="divide-y divide-slate-50/50">
                            {transactions.filter(t => t.referenceNo?.toLowerCase().includes(searchTerm.toLowerCase()) || t.supplier?.name.toLowerCase().includes(searchTerm.toLowerCase())).map((t, idx) => (
                                <tr key={t._id} className="group hover:bg-slate-50/30 transition-colors">
                                    ...
                                    <td className="py-4 px-4 text-xs font-bold text-slate-400">{t.supplier?.name}</td>
                                    ...
                                    <td className="py-4 px-4 text-xs font-bold text-emerald-500">₹{t.price * t.quantity}</td> { /* Placeholder logic for paid */}
                                    <td className="py-4 px-4 text-xs font-bold text-slate-400">0.00</td> { /* Placeholder logic for balance */}
                                    <td className="py-4 px-4">
                                        <span className="px-3 py-1 bg-emerald-50 text-emerald-500 rounded-full text-[10px] font-black uppercase tracking-wider">
                                            {(t.price * t.quantity) > 0 ? 'PAID' : 'PENDING'}
                                        </span>
                                    </td>
                            ...
                            {transactions.length === 0 && (
                                <tr>
                                    <td colSpan="10" className="py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">
