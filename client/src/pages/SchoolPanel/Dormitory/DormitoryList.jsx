...
const DormitoryList = () => {
    ...
    const fetchDormitories = async () => {
        try {
            const { data } = await api.get('/api/dormitory');
            setDormitories(data);
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/api/dormitory/${id}`);
            fetchDormitories();
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex justify-between items-center px-4">
                <div>
                    <h1 className="text-xl font-black text-slate-800 tracking-tight">Dormitory List</h1>
                </div>
                <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <span>|</span>
                    <span className="text-slate-500">Dormitory List</span>
                </div>
            </div>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {dormitories.map((dorm, idx) => (
                                    <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="py-6 px-6 text-slate-400 text-xs">{dorm.name}</td>
                                        <td className="py-6 px-6 text-slate-400 text-xs">{dorm.type}</td>
                                        <td className="py-6 px-6 text-slate-400 text-xs">{dorm.address}</td>
                                        <td className="py-6 px-6 text-slate-400 text-xs">{dorm.intake}</td>
