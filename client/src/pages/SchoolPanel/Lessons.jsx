...
import { Search, Plus, Trash2, CheckCircle2, BookOpen, GraduationCap, ChevronDown } from 'lucide-react';
...
const Lessons = () => {
    ...
    const fetchData = async () => {
        try {
            const [lessonRes, classRes, subjectRes] = await Promise.all([
    ...
    const handleLessonNameChange = (index, value) => {
        const newLessonNames = [...formData.lessonNames];
        newLessonNames[index] = value;
        setFormData({ ...formData, lessonNames: newLessonNames });
    };
    ...
    const removeLessonField = (index) => {
        const newLessonNames = formData.lessonNames.filter((_, i) => i !== index);
        setFormData({ ...formData, lessonNames: newLessonNames });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            // Send each lesson name as a separate request or modify backend to accept array
            // For now, let's assume we send them one by one or the backend handles array.
            // Based on previous code, backend expects single object.
            // We will iterate and send multiple requests for now to ensure stability without backend changes,
            // or better: update the backend to handle bulk.
            // User asked for UI change mainly. I will loop requests here for simplicity.

            for (const name of formData.lessonNames) {
                if (name.trim()) {
                    await api.post('/api/lesson-plan/lessons', {
            ...
            setFormData({ class: '', section: '', subject: '', lessonNames: [''] });
            fetchData();
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/api/lesson-plan/lessons/${id}`);
            fetchData();
        } catch (error) {
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-12 text-left">
            <div className="flex justify-between items-center px-2">
                ...
                <div className="hidden md:flex space-x-2 text-[10px] font-bold text-slate-400">
                    ...
                    <span>|</span>
                    <span className="text-primary/70">Lesson</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <Card className="p-6 border-none shadow-snow-lg rounded-[20px] bg-white sticky top-24">
                        <h2 className="text-[#3E4D67] text-base font-bold mb-6">Add Lesson</h2>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="space-y-3 pt-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Add Lesson Name</label>
                                {formData.lessonNames.map((name, index) => (
                                    <div key={index} className="flex gap-2">
                                        <input
                                            ...
                                            required
                                        />
                                        {index === formData.lessonNames.length - 1 ? (
                                            <button
                                                type="button"
                ...
                <div className="lg:col-span-2">
                    <Card className="p-6 border-none shadow-snow-lg rounded-[20px] bg-white overflow-hidden min-h-[500px]">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                            <h2 className="text-base font-bold text-[#3E4D67]">Lesson List</h2>
                                ...
                                <tbody className="divide-y divide-slate-50 text-[11px] font-bold">
                                    {filteredLessons.map((item, i) => (
                                        <tr key={item._id} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-slate-600">{item.section || 'A'}</td>
                                            <td className="px-4 py-3 text-slate-500 italic">{item.subject?.name}</td>
                                            <td className="px-4 py-3 text-slate-700">{item.lessonName}</td>
                                            <td className="px-4 py-3 text-right">
                                                <div className="flex justify-end items-center space-x-2">
                                                    <button className="px-3 py-1 bg-white border border-[#7c32ff] text-[#7c32ff] rounded-full text-[9px] font-black uppercase tracking-wider hover:bg-[#7c32ff] hover:text-white transition-all">
                                                        Select <ChevronDown size={10} className="inline ml-1" />
                                                    </button>
                                    ...
                                    {filteredLessons.length === 0 && (
                                        <tr>
                                            <td colSpan="6" className="px-6 py-10 text-center text-slate-400 italic font-medium text-xs">
                                                No Lessons found
                                            </td>
                                        </tr>
                                    )}
