...
import { Search, Plus, Trash2, MoreVertical, CheckCircle2, ChevronDown, User } from 'lucide-react';
...
const AssignClassTeacher = () => {
    ...
    const fetchData = async () => {
        try {
            const [assignRes, classRes, sectionRes, teacherRes] = await Promise.all([
                api.get('/api/academic/class-teachers'),
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.classId || !formData.sectionName || !formData.teacherId) {
            alert('Please fill all required fields');
            return;
        }
        setLoading(true);
        try {
            await api.post('/api/academic/class-teachers', {
                class: formData.classId,
                section: formData.sectionName,
                teacher: formData.teacherId
            });
            setFormData({ classId: '', sectionName: '', teacherId: '' });
            fetchData();
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/api/academic/class-teachers/${id}`);
            fetchData();
        } catch (error) {
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-12">
            <div className="flex justify-between items-center px-2">
                ...
                <div className="hidden md:flex space-x-2 text-[10px] font-bold text-slate-400">
                    ...
                    <span>Academics</span>
                    <span>|</span>
                    <span className="text-primary/70">Assign Class Teacher</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-1">
                    <Card className="p-8 border-none shadow-snow-lg rounded-[20px] bg-white sticky top-24">
                        <h2 className="text-[#3E4D67] text-lg font-bold mb-8">Assign Class Teacher</h2>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex justify-center pt-6">
                                ...
                                >
                                    ...
                                    <span>{loading ? 'Saving...' : 'Save Class Teacher'}</span>
                                </Button>
                            </div>
                        </form>
                    </Card>
                ...
                <div className="lg:col-span-3">
                    <Card className="p-8 border-none shadow-snow-lg rounded-[20px] bg-white overflow-hidden">
                        <div className="flex justify-between items-center mb-8 px-2">
                            <h2 className="text-lg font-bold text-[#3E4D67]">Class Teacher List</h2>
                            <div className="relative group">
                                <input
                                    type="text"
                                    placeholder="SEARCH"
