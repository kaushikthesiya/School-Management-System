...
const PostalDispatch = () => {
    ...
    const fetchDispatches = async () => {
        setLoading(true);
        try {
            const { data } = await api.get('/api/admin-section/postals/dispatch');
            setDispatches(data);
        } catch (error) {
            console.error('Fetch Postal Dispatch Error:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showToast('Postal dispatch saved successfully!');
            setFormData({
                toTitle: '',
                referenceNo: '',
                address: '',
            ...
            showToast('Error saving postal dispatch', 'error');
            console.error('Add Postal Dispatch Error:', error);
        }
    };

    const filteredDispatches = dispatches.filter(item =>
        item.toTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-12">
            <div className="flex justify-between items-center px-2">
                ...
                <div className="hidden md:flex space-x-2 text-[10px] font-bold text-slate-400">
                    ...
                    <span>|</span>
                    <span className="text-primary/70">Postal Dispatch</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <Card className="lg:col-span-4 p-8 border-none shadow-snow-lg rounded-[20px] bg-white">
                    <h2 className="text-[#3E4D67] text-lg font-bold mb-8">Add Postal Dispatch</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex justify-center pt-4">
                            <Button type="submit" className="!bg-[#7c32ff] !rounded-lg w-full flex items-center justify-center space-x-2 shadow-lg shadow-purple-500/20 active:scale-95 transition-all">
                                <Check size={18} />
                                <span className="uppercase text-xs font-bold tracking-widest">Save Postal Dispatch</span>
                            </Button>
                        </div>
                    </form>
                </Card>
                ...
                <Card className="lg:col-span-8 p-0 border-none shadow-snow rounded-[20px] overflow-hidden bg-white min-h-[600px]">
                    <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 text-center md:text-left">
                        <h2 className="text-lg font-bold text-[#3E4D67]">Postal Dispatch List</h2>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/50">
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest italic">To Title</th>
                                    ...
                                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest italic">From Title</th>
                                ...
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-8 py-20 text-center text-slate-400 italic font-medium">
                                            No postal dispatches found.
                                        </td>
                                    </tr>
                                ) : (
                                    filteredDispatches.map((item) => (
                                        <tr key={item._id} className="group hover:bg-slate-50/30 transition-colors">
                                            <td className="px-8 py-5 text-sm font-black text-slate-700 italic uppercase">{item.toTitle}</td>
                                            ...
                                            <td className="px-6 py-5 text-sm font-black text-slate-700 italic uppercase">{item.fromTitle}</td>
