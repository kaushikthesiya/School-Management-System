...
const IncidentWiseReport = () => {
    ...
    const fetchReport = async () => {
        setSearching(true);
        try {
            const { data } = await api.get('/api/discipline/report/by-category');
            setReportData(data);
        } catch (err) {
            console.error('Fetch report error:', err);
            alert('Failed to fetch incident report');
        } finally {
            setSearching(false);
        }
    };
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-50 italic">
                                <th className="text-left py-5 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Incident Name (Category)</th>
                                <th className="text-right py-5 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Reports</th>
                            </tr>
                        </thead>
                        <tbody>
                            {searching ? (
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-all border-b border-slate-50 last:border-0">
                                    <td className="py-5 px-6 text-[11px] font-bold text-slate-600 italic uppercase tracking-wider">{row._id}</td>
                                    <td className="py-5 px-6 text-[11px] font-bold text-[#7c32ff] tabular-nums text-right italic">{row.count}</td>
                                </tr>
                            )) : (
                                <tr>
