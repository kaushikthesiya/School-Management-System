...
const Designation = () => {
    ...
    const fetchDesignations = async () => {
        setLoading(true);
        try {
            const { data } = await api.get('/api/admin-section/designations');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await api.patch(`/api/admin-section/designations/${editingId}`, formData);
                showToast('Designation updated successfully');
            } else {
                await api.post('/api/admin-section/designations', formData);
            ...
            showToast(error.response?.data?.message || 'Error saving designation', 'error');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/api/admin-section/designations/${id}`);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-12 text-slate-800">
            ...
            <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-xl font-black text-slate-800 tracking-tight italic uppercase">Designation</h1>
                    <div className="flex space-x-2 text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">
                        <span>Dashboard</span>
                        <span>|</span>
                        <span>Human Resource</span>
                        <span>|</span>
                        <span className="text-[#1C1C1C]">Designation</span>
                    </div>
                </div>
            </div>
                    ...
                    <Card className="p-0 border-none shadow-3xl shadow-slate-100 bg-white overflow-hidden rounded-3xl sticky top-24">
                        <div className="bg-slate-50/50 p-6 border-b border-slate-50">
                            <h2 className="text-[10px] font-black text-[#1C1C1C] uppercase tracking-[0.2em] flex items-center space-x-3">
                                ...
                                <span>{editingId ? 'Edit Designation' : 'Add Designation'}</span>
                            </h2>
                        </div>
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            ...
                            <div className="pt-2 flex flex-col items-center space-y-4">
                                ...
                                >
                                    ...
                                    <span>{saving ? 'SAVING...' : editingId ? 'UPDATE DESIGNATION' : 'SAVE DESIGNATION'}</span>
                                </Button>
                                {editingId && (
                                    <button
                                        type="button"
                ...
                <div className="lg:col-span-8">
                    <Card className="p-8 border-none shadow-3xl shadow-slate-100 bg-white rounded-3xl">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 px-2">
                            <h3 className="text-sm font-black text-navy-900 uppercase tracking-widest italic">Designation List</h3>
